                ...
                <tbody>
                    @forelse ($attributes as $index => $attribute)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('attributes.show', $attribute->id) }}" class="btn btn-sm btn-info">Show</a>
                                <a href="{{ route('attributes.edit', $attribute->id) }}" class="btn btn-sm btn-warning">Edit</a>
                                <form action="{{ route('attributes.destroy', $attribute->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('Are you sure?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="6" class="text-center text-muted">No attributes found.</td></tr>
                    @endforelse
                </tbody>
            </table>
