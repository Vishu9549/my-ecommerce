      ...
      <div className="container">
        ...
        <div className="collapse navbar-collapse" id="navbarsFurni">
            ...
            {categories.map((category) => (
              <li key={category.id} className="nav-item dropdown">
                ...
                {category.children && category.children.length > 0 && (
                  <ul className="dropdown-menu bg-dark" aria-labelledby={`navbarDropdown-${category.id}`}>
                    {category.children.map((child) => (
                      <li key={child.id}>
                        <Link className="dropdown-item" to={`/categories/${category.slug}/${child.slug}`}>
                          {child.name}
                        </Link>
                      </li>
                    ))}
          ...
          <ul className="custom-navbar-cta navbar-nav mb-2 mb-md-0 ms-5 align-items-center">
            {/* User Icon with Name/New User */}
            <li className="text-center me-3">
              <Link className="nav-link p-0" to="/account">
                <img src="/images/user.svg" alt="user" style={{ width: '24px' }} />
                <div className="text-white" style={{ fontSize: '0.8rem', marginTop: '2px' }}>
